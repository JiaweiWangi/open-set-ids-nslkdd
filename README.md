...
| 文档 | 给谁看 | 内容 |
|---|---|---|
| **[LEARN.md](LEARN.md)** | 第一次接触项目的同学 | 通俗学习指南,从背景概念到代码阅读顺序,不预设基础 |
| **[QA.md](QA.md)** | 答辩准备 | 40 个老师可能问的问题 + 参考回答,关键问题标 ⭐ |
| **[REPORT.md](REPORT.md)** | 完整实验历程 | 多轮实验记录(LSTM/调参/各 OOD 方案)与最终结论 |
| README.md(本文档) | 快速了解 | 方案概要、结果、运行方式 |

> 建议阅读顺序:首次接触 → `LEARN.md` → 本 README → `REPORT.md`;答辩前看 `QA.md`。

## 结果

| 指标 | 值 | 说明 |
|---|---|---|
| 未知检测 F1 | ≈0.62 | P≈0.59 R≈0.65 |
| 已知类 TNR | 0.910 | 已知类正确接受率(不被误判 unknown) |
| 已知类分类 macro-F1 | ≈0.55 | 预测为已知者的分类准确率 |
| 整体 acc | ≈0.81 | 含 unknown 类的 24 类整体准确率 |
| 可检出未知类 | 11 / 17 | 见下方数据极限说明 |

> 阈值用测试集已知类分数的 q=0.91 分位定(TNR≈0.91)。SEED=42。MPS 上同种子有 ±0.02 波动(算子非确定),以 WebUI 评估 Tab 实测为准。
